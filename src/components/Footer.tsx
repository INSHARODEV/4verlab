        ...
        <footer id="contact" className="bg-surface-dark text-white border-t border-white/10">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-12">
                    ...
                    {/* Navigation */}
                    <div>
                        <h3 className={`text-sm font-semibold uppercase tracking-wider text-white/40 mb-6 ${isRtl ? 'text-right' : 'text-left'}`}>
                        ...
                        <nav className="flex flex-col gap-3">
                            {navLinks.map((link) => (
                                <Link
                                    key={link.href}
                                    href={link.href}
                                    className={`text-white/70 hover:text-primary transition-colors text-sm ${isRtl ? 'text-right' : 'text-left'}`}
                                >
                                    {link.label}
                                </Link>
                            ))}
                ...
                <div className="mt-16 pt-8 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-6">
                    <p className={`text-white/40 text-sm text-center ${isRtl ? 'sm:text-right' : 'sm:text-left'}`}>
                ...
                <div className="mt-12 flex flex-wrap justify-center items-center gap-8">
                    ...
                    <div className={`flex items-center gap-2 ${isRtl ? 'flex-row' : 'flex-row-reverse'}`}>
                        <p className={`text-white/60 text-sm ${isRtl ? 'text-right' : 'text-left'}`}>{t.footer.businessInfo}<br />7053180589</p>
